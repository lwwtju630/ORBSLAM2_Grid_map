...
int main(int argc, char **argv){
	...
	message_filters::Subscriber<sensor_msgs::Image> rgb_sub(nodeHandler, rgb_topic, 1);
...
bool parseParams(int argc, char **argv) {
	...
	if (argc < 3){
		cerr << endl << "Usage: rosrun ORB_SLAM2 Monopub path_to_vocabulary path_to_settings" << endl;
		return 0;
	}
	if (argc >= 4) {
		all_pts_pub_gap = atoi(argv[3]);
	}
